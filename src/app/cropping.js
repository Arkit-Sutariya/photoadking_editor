...
s))(_ || {});
function ue(s) {
    let {pointer: e, croppedData: t, croppedControlCoords: r, sourceData: i, sourceControlCoords: o, corner: n, freeModeScaling: b } = s,
    a = t.angle,
    l = r[_[n]],
    h = e.subtract(l).rotate(-a),
    c = new g(o[n]).subtract(l).rotate(-a),
    // For freeform scalling
    ...
    //   , aspectRatio = Math.abs(c.y) / Math.abs(c.x)
    //   , p = Math.sign(h.x) === Math.sign(c.x) ? Math.abs(h.x) : 0
    //   , m = p * aspectRatio
    //   , d = ee(p, 16, Math.abs(c.x))
    //   , f = ee(m, 16, Math.abs(c.y))
